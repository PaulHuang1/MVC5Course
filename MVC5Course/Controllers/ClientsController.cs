...
namespace MVC5Course.Controllers
{
    public class ClientsController : BaseController
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(int id, FormCollection form)  //FormCollection 參數無用，只是為了跟Form 的edit作區別
        {
            var client = db.Client.Find(id);
            if (TryUpdateModel(client, null, null, excludeProperties : new string[] { "IsAdmin"}))
            {
                db.SaveChanges();
                return RedirectToAction("Index");
            }
